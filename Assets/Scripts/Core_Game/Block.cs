...
public class Block : MonoBehaviour
{
    ...
    public void SetCanSpawnPowerUpBlock (bool canSpawnPowerUpBlock) { this.canSpawnPowerUpBlock = canSpawnPowerUpBlock; }
    ...
    private void Start () 
    {
        ...
        if (hasRandomHits)
        {
            maxHits = (int) Random.Range (1, hitSprites.Length + 2);
        }
        else 
        {
    ...
    private void DestroyBlock ()
    {
        ...
        if (collidedWithBall) { gameSession.AddToComboMultiplier (); }
        ...
        if (collidedWithBall)
        {
            if (comboMultiplier > 1) { score *= comboMultiplier; }
        }
