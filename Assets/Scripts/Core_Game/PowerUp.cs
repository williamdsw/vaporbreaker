...
public class PowerUp : MonoBehaviour
{
    ...
    private void Update ()
    {
        ...
        {
            if (canRotateChance >= 50) { RotateObject (); }
        }
    }
    ...
    private void OnCollisionEnter2D (Collision2D other) 
    {
        if (gameSession.GetActualGameState () == GameState.GAMEPLAY)
        {
            ...
            if (paddle) { DealCollisionWithPaddle (); }
        }
    }

    //--------------------------------------------------------------------------------//

    // Adds a random force to rigid body
    ...
    private void ApplyPowerUpEffect (string powerUpName)
    {
        switch (powerUpName)
        {
            case "PowerUp_All_Blocks_1_Hit": { MakeAllBlocksOneHit (); currentPowerUpIndex = 0; break; }
            case "PowerUp_Ball_Bigger": { DefineBallsSize (true); currentPowerUpIndex = 1;  break; }
            case "PowerUp_Ball_Smaller": { DefineBallsSize (false); currentPowerUpIndex = 2; break; }
            case "PowerUp_Ball_Faster": { DefineBallsSpeed (true); currentPowerUpIndex = 3; break; }
            case "PowerUp_Ball_Slower": { DefineBallsSpeed (false); currentPowerUpIndex = 4; break; }
            case "PowerUp_Duplicate_Ball": { DuplicateBalls (); currentPowerUpIndex = 5; break; }
            case "PowerUp_Paddle_Expand": { if (paddle) { paddle.DefinePaddleSize (true); currentPowerUpIndex = 6; } break; }
            case "PowerUp_Paddle_Shrink": { if (paddle) { paddle.DefinePaddleSize (false); currentPowerUpIndex = 7; } break; }
            case "PowerUp_Level_Complete": { gameSession.CallLevelComplete (); currentPowerUpIndex = 8; break; }
            case "PowerUp_Random": { ApplyPowerUpEffect (powerUpNames[Random.Range (0, powerUpNames.Length)]); break; }
            case "PowerUp_Reset_Ball": { ResetBalls (); currentPowerUpIndex = 9; break; }
            case "PowerUp_Reset_Paddle": { if (paddle) { paddle.ResetPaddle (); currentPowerUpIndex = 10; } break; }
            case "PowerUp_Shooter": { CreateShooter (); currentPowerUpIndex = 11; break; }
            case "PowerUp_Unbreakables_To_Breakables": { MakeUnbreakableToBreakable (); currentPowerUpIndex = 12; break; }
            case "PowerUp_Zero_Deaths": { gameSession.ZeroNumberOfDeaths (); currentPowerUpIndex = 13; break; }
            default: { break; }
        }
    }

    // Sets the power up name to game session
    ...
    private void MakeAllBlocksOneHit ()
    {
        ...
        if (blocks.Length != 0)
        {
            foreach (Block block in blocks) { block.SetMaxHits (1); }
        }
    }
        ...
        if (balls.Length != 0)
        {
            foreach (Ball ball in balls)
            {
                if (ball.GetIsBallFree ())
                {
                    ...
                    if (makeBigger) { if (newLocalScale.x < ball.GetMaxBallLocalScale ()) { newLocalScale *= 2f; } }
                    else  { if (newLocalScale.x > ball.GetMinBallLocalScale ()) { newLocalScale /= 2f; } }
        ...
        if (balls.Length != 0)
        {
            foreach (Ball ball in balls)
            {
                if (ball.GetIsBallFree ())
                {
                    ...
                    if (moveFaster)
                    {
                        if (moveSpeed < ball.GetMaxMoveSpeed ()) { moveSpeed += 100f; }
                        if (ballRotationDegree < ball.GetMaxBallRotationDegree ()) { ballRotationDegree *= 2; }
                    }
                    else 
                    {
                        if (moveSpeed > ball.GetMinMoveSpeed ()) { moveSpeed -= 100f; }
                        if (ballRotationDegree > ball.GetMinBallRotationDegree ()) { ballRotationDegree /= 2; }
                    }
